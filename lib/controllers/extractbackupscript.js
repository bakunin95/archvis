...
fs.readFile("website/www.lapresse.ca/index.html","utf-8", function read(err, data) {
    if (err) {
        throw err;
    }
	var filters = {
	...
	var result = assetment( data, filters );
res.send("done");
	 //res.send(result);
});
*/
...
new AssetGraph({root:  '/website/test/'})
...
//console.log(htmlAsset);
	//var htmlwithoutCirc = removeCircular(assetGraph._assets[1]);
  //  res.send(htmlwithoutCirc);

	_.each(htmlAsset, function (v, k) {
	   console.log(v);
...
//console.log(cropedRes);
	/*fs.writeFile("extracted.json", stringify(htmlwithoutCirc,0,6), function(err) {
	    if(err) {
	        console.log(err);
	    } else {
