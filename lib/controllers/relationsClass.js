var fs = require("fs"),
    ...
    function findingAssets(html){
    	var assets = {};
    	//{"stylesheets","javascripts","hyperlink"}
    ...
    function testasync(str,callb){
    	console.log("testasync:",str);
    	callb();
    }
	...
	function findAssets(assets,callbackFindAssets){
		...
		async.parallel({
		    relationsJavaScript: function(cbJs){
		    	...
		    	async.map(assets.javascripts, function(currentAsset,cb) {
	    	...
	    	async.waterfall([
			    ...
			    function(htmlFileList,callbackAssets){
			    	...
			    	async.map(htmlFileList, function(filePath,callbackTrouverAssets){
			    		...
			    		//testasync(filePath,callbackTrouverAssets);


			    										
			    		readAsync(filePath,function(err,result){
			    				...
			    				try{
				    				var assets = findingAssets(result[1]);
				    				relationsClass.addNodes({"name":filePath,"group":1});
									relationsClass.targetCount = relationsClass.sourceCount-1;

									//testasync(filePath,callbackTrouverAssets);
									findAssets(assets,callbackTrouverAssets);
